...
class Property(models.Model):
    ...
    def assign_total_loan_payment(self, interest_change_year=None, new_interest_rate=None):
    ...
    def assign_pre_tax_cashoncash(self):
        for i in range(30):
            cash_on_cash = self.pre_tax_cashflow_list[i]/self.initial_capital_outflow_list[i] * 100
    ...
    def assign_taxable_deductions(self):
        for i in range(30):
            deductions = self.loan_interest_list[i] + self.total_property_expenses_list[i]
    ...
    def assign_after_tax_cashoncash(self):      
        for i in range(30):
            cash_on_cash = (self.after_tax_cashflow_list[i] / self.initial_capital_outflow_list[i]) * 100
...
class Images(models.Model):
    image = CloudinaryField('images', default='http://res.cloudinary.com/dim8pysls/image/upload/v1639001486''/x3mgnqmbi73lten4ewzv.png')
...
class RentalIncome(models.Model):
    rentalincreasetype = models.CharField(('Type'),null=True,max_length=50,default='capital', choices=(
        ('capital', 'capital'),
        ('inflation', 'inflation'),
        ('percent', 'percent'),
    ))
    ...
    amount = ArrayField(models.IntegerField(), default=list) # amount = models.IntegerField(('Amount'),null=True)
    property = models.ForeignKey(Property, on_delete=models.CASCADE, null=True)
    def __str__(self):
